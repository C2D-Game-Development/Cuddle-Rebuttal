...
function drawFloorBarrier(canvasDisplay) {
  let { img, x, y, w, h } = canvasDisplay.createFloor;
  //ctx.fillRect(0, 0, 100, 100)
  ctx.drawImage(images[img], x, y, w, h);
}

function drawLeftBarrier(canvasDisplay) {
  let { img, x, y, w, h } = canvasDisplay.createLeftWall;
  ctx.drawImage(images[img], x, y, w, h);
}
...
socket.on("displayCanvas", ({ canvasDisplay }) => {
  ...
  drawFloorBarrier(canvasDisplay); //floor
  drawLeftBarrier(canvasDisplay); //left wall
  drawPlayer(canvasDisplay.createPlayer1); //player 1
  drawPlayer(canvasDisplay.createPlayer2); //player 2
});
