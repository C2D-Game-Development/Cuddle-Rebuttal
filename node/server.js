...
class Player {
  constructor(x, y, w, h, img, rImg, direction) {
    ...
    this.sy = 1530//img.height / 5; //animation type
    this.sw = 580//img.width / 10;
    this.sh = 510//img.height / 5;
    this.numberTall = 5;
    this.numberWide = 10;
    this.health = 100;
    this.special = 0;
...
class CanvasDisplay {
  ...
  animate() {
    ...
    console.log(yourId, firstUser)
    //if(userId === firstUser) { 
    // socket.emit('sendGameState', { state: this })
    //}
...
function moveThings() {
  ...
  io.sockets.emit('displayCanvas', { canvasDisplay })
}


const keys = []

setInterval(moveThings, 15)
...
io.on("connection", (socket) => {
  ...
  socket.emit("yourId", { id: socket.id, firstUser: firstUser });
  ...
  //   io.sockets.emit('drawGameState', { state })
  // })
